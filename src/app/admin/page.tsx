  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
      <div className="bg-gray-50 px-3 py-2 text-xs font-bold text-gray-700 flex justify-between items-center border-b border-gray-100">
        <span className="truncate max-w-[150px] capitalize" title={label}>{label}</span>
...
export default function AdminDashboard() {
  ...
  // --- LOGIC TO MERGE & CLEAN DOCUMENTS ---
  const getUniqueDocuments = (app: Application) => {
    const docs: { url: string, label: string }[] = [];
    const seenUrls = new Set<string>();

    // 1. Priority: Custom Fields (Because they have labels like "ID Card")
    if (app.custom_fields) {
      Object.entries(app.custom_fields).forEach(([key, value]) => {
        if (typeof value === 'string' && value.startsWith('http')) {
          docs.push({ url: value, label: key.replace(/_/g, ' ') });
          seenUrls.add(value);
        }
      });
    }

    // 2. Fallback: The 'documents' array (Only add if we haven't seen this URL yet)
    if (app.documents) {
      app.documents.forEach((url, idx) => {
        if (!seenUrls.has(url)) {
          docs.push({ url, label: `Attachment ${idx + 1}` });
          seenUrls.add(url);
        }
      });
    }
  ...
  const addField = () => {
    setEditingService((prev: any) => ({
      ...prev,
      form_fields: [...(prev.form_fields || []), { id: `field_${Date.now()}`, label: 'New Field', type: 'text', required: true }]
    }));
  };
  ...
  const updateField = (index: number, key: string, value: any) => {
    setEditingService((prev: any) => {
      ...
      if (key === 'label') newFields[index].id = value.toLowerCase().replace(/ /g, '_').replace(/[^a-z0-9_]/g, '');
    ...
    <div className="min-h-screen bg-gray-50 pt-24 px-6 pb-20">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-end mb-8">
          <div><h1 className="text-2xl font-bold text-gray-900">Admin Command</h1><p className="text-gray-500 text-sm">Manage Orders & Catalog</p></div>
          {activeTab === 'services' && (
            <button onClick={() => setEditingService({ category: 'General', govt_cost: 0, service_fee: 150, form_fields: [] })} className="bg-black text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center hover:bg-gray-800 transition-colors">
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {applications.length === 0 ? (
                        <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400 italic">No orders found.</td></tr>
                    ) : (
                        ...
                        <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-sm font-bold">{app.price_paid === 0 ? <span className="text-gray-400 text-[10px]">QUOTE PENDING</span> : `KES ${(app.price_paid || 0).toLocaleString()}`}</td>
                            <td className="px-6 py-4 text-right"><button onClick={() => openAppDetails(app)} className="text-blue-600 font-bold text-xs hover:underline">Manage</button></td>
                        </tr>
                        ))
                    )}
                  </tbody>
                  ...
                  return (
                    <div key={svc.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm group hover:border-black transition-all">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <button onClick={() => setEditingService(svc)} className="text-gray-400 hover:text-black transition-colors"><Edit2 className="h-4 w-4" /></button>
                      </div>
                      ...
                      <div className="mt-auto border-t border-gray-100 pt-3">
                        <div className="flex justify-between items-end mb-2"><span className="text-xs text-gray-400">Total Customer Price:</span><span className="font-bold text-black">KES {total.toLocaleString()}</span></div>
                        <div className="bg-gray-50 p-2 rounded-lg flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                             <input type="number" value={svc.service_fee} onChange={(e) => handleFeeUpdate(svc, parseInt(e.target.value))} className="w-20 border border-gray-200 rounded px-2 py-1 text-xs font-bold focus:outline-none focus:border-black text-right" />
                            <button onClick={() => saveQuickFee(svc)} className="text-[10px] bg-black text-white px-2 py-1 rounded hover:bg-gray-800 transition-colors">Save</button>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        {editingService && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl p-6 max-h-[90vh] overflow-y-auto flex flex-col">
              <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold">{editingService.id ? 'Edit Service' : 'New Service'}</h2><button onClick={() => setEditingService(null)}><X className="h-5 w-5" /></button></div>
              <div className="space-y-4 mb-8">
                <div><label className="text-xs font-bold uppercase text-gray-500">Service Title</label><input className="w-full border p-2 rounded-lg" value={editingService.title || ''} onChange={e => setEditingService({...editingService, title: e.target.value})} /></div>
                <div><label className="text-xs font-bold uppercase text-gray-500">Category</label><input className="w-full border p-2 rounded-lg" value={editingService.category || ''} onChange={e => setEditingService({...editingService, category: e.target.value})} /></div>
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                    <div className="flex items-center gap-2 mb-3"><Calculator className="h-4 w-4 text-blue-600" /><h3 className="text-sm font-bold text-blue-900">Price Configuration</h3></div>
                    <div className="grid grid-cols-3 gap-4">
                        <div><label className="text-[10px] font-bold uppercase text-gray-500">Govt Official Cost</label><input type="number" className="w-full border border-gray-300 p-2 rounded-lg bg-white" value={editingService.govt_cost || 0} onChange={e => setEditingService({...editingService, govt_cost: parseInt(e.target.value) || 0})} /></div>
                        <div><label className="text-[10px] font-bold uppercase text-gray-500">My Service Fee</label><input type="number" className="w-full border border-gray-300 p-2 rounded-lg bg-white font-bold text-black" value={editingService.service_fee || 0} onChange={e => setEditingService({...editingService, service_fee: parseInt(e.target.value) || 0})} /></div>
                         <div><label className="text-[10px] font-bold uppercase text-gray-400">Total Customer Price</label><div className="w-full bg-gray-200 border border-gray-200 p-2 rounded-lg text-gray-500 font-bold">KES {((editingService.govt_cost || 0) + (editingService.service_fee || 0)).toLocaleString()}</div></div>
                    </div>
                </div>
                <div><label className="text-xs font-bold uppercase text-gray-500">Description</label><textarea className="w-full border p-2 rounded-lg h-20" value={editingService.description || ''} onChange={e => setEditingService({...editingService, description: e.target.value})} /></div>
              </div>
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                 <div className="flex justify-between items-center mb-4"><h3 className="text-sm font-bold uppercase text-gray-700">Requirements & Fields</h3><button onClick={addField} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg flex items-center hover:bg-blue-700"><Plus className="h-3 w-3 mr-1" /> Add Field</button></div>
                 <div className="space-y-3">
                    {editingService.form_fields?.map((field: FormField, idx: number) => (
                      <div key={idx} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm flex items-center gap-3">
                         ...
                         <div className="flex-grow"><input placeholder="Label" className="w-full text-sm font-medium focus:outline-none border-b border-transparent focus:border-blue-500" value={field.label} onChange={(e) => updateField(idx, 'label', e.target.value)} /></div>
                         <select className="text-xs bg-gray-50 border border-gray-200 rounded px-2 py-1" value={field.type} onChange={(e) => updateField(idx, 'type', e.target.value)}><option value="text">Text Input</option><option value="file">File Upload</option><option value="date">Date Picker</option><option value="number">Number</option></select>
                         <button onClick={() => updateField(idx, 'required', !field.required)} className={`text-xs px-2 py-1 rounded font-bold border ${field.required ? 'bg-red-50 text-red-600 border-red-200' : 'bg-gray-50 text-gray-400 border-gray-200'}`}>{field.required ? 'Required' : 'Optional'}</button>
                         <button onClick={() => removeField(idx)} className="text-gray-400 hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    ))}
                 </div>
              </div>
              <div className="mt-6 flex justify-end"><button onClick={handleSaveService} disabled={isUpdating} className="w-full bg-black text-white font-bold py-3 rounded-xl hover:bg-gray-800 flex justify-center items-center transition-all">{isUpdating ? <Loader2 className="animate-spin h-5 w-5" /> : 'Save Service Changes'}</button></div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-y-0 right-0 w-full max-w-2xl bg-white shadow-2xl z-50 flex flex-col h-full">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8">
                ...
                <div>
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      {getUniqueDocuments(selectedApp).map((doc, i) => (
                        <DocumentPreview key={i} url={doc.url} label={doc.label} />
                      ))}
                      {getUniqueDocuments(selectedApp).length === 0 && (
